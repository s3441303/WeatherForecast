...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"ShowWeather"]) {
        WeatherForecastViewController *wvc = [segue destinationViewController];
        NSIndexPath *path =[self.tableView indexPathForSelectedRow];
        Location *c = [locations objectAtIndex:[path row]];
        [wvc setCurrentLocation:c];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell...
    Location *current = [locations objectAtIndex:indexPath.row];
    [cell.textLabel setText:[current city]];
    return cell;
}
